...
def update_map(hover_data):
    fig = px.choropleth(
        ...
        # Use shades of green for the color scale
    ...
    fig.update_layout(
        ...
        # Center-align the color bar (legend) title
        coloraxis_colorbar=dict(
            title="Carbon Intensity",
            titlefont=dict(size=16, family="Roboto", color=colors['text']),
            tickfont=dict(size=14, family="Roboto", color=colors['text']),
            titleside='top',  # Place title at the top of the color bar
            xanchor='center',  # Center-align
            x=0.5  # Adjust position to center
        )
    )
